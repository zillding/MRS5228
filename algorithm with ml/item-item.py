...
def machineLearning(weight,user_rating,predicted_rating,true_result):
    predicted_rating = float(predicted_rating)
    true_result = float(true_result)
    for product_id in user_rating.index:
        if not product_id in weight.index:
            continue
        #distribute result as like and dislike 
        #>=3 => like
        #<3 => dislike
        mispredict = 0

        if (abs(predicted_rating - true_result) >=2):
            mispredict = 1
        else:
            mispredict = 0
        ...
        w = float(weight.loc[product_id])
        u = float(user_rating.loc[product_id])

        #perceptron learning process
        # weight.loc[product_id] = w + learning_rate*u
        # logger.debug('weight for product_id {} has been adjust from {} to {}'.format(product_id,w,weight.loc[product_id]))
        if predicted_rating > true_result:
            #false positive
            weight.loc[product_id] = w - learning_rate*u
            logger.debug('weight for product_id {} has been adjust from {} to {}'.format(product_id,w,weight.loc[product_id]))
        else:
...
for i in range(number_of_data_sets):
    ...
    for target_productid in test_productid_array:
        ...
        for userid in test_df[test_df.product_productid == target_productid].review_userid.unique():
            ...
            user_ratings = user_df.loc[knn.index]
            ...
            # machine learning
            if float(predict_rating - actual_rating) != 0:
                machineLearning(result_sim,user_ratings,predict_rating,actual_rating)
